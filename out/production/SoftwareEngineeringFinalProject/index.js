...
app.get('/searchQuestion', async (req, res) => {
    ...
    // Find the index of the found question
    ...
    if (index !== -1) {
        ...
        // Construct the edit and delete endpoints
...
app.use('/deleteAnswer', (req, res) => {
    var filter = { 'answerQuestion' : req.query.answerQuestion };
    Answer.findOneAndDelete( filter, (err, ans) => {
        if(err) {
            //res.json({ 'status' : err });
            ...
            res.type('html').status(200);
            res.write('<li>');
            res.write("User profile " + ans.answerQuestion + " successfully deleted. ");
            res.write('<ul>')
